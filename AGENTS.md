...
9. **Mandatory implementation workflow**: 每次改代码之前必须走完以下工作流，不得跳步：
   1. **Grill with Docs** — 用 `grill-with-docs` skill 审视方案，确认设计决策有文档支撑。**必须主动做场景风险分析**：按 `docs/best-practices/scenario-enumeration-checklist.md` 逐类检查边界场景（含跨 step 接口契约验证），验证跨包/跨消费者一致性。涉及内存缓存的改动另参 `docs/memory-leak-checklist.md`。
