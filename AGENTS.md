...
- `.github/workflows/*.yml` pin third-party actions with full 40-character commit SHAs in `uses:` and a trailing `# vN` (or similar) comment for the human-readable release; upgrade by changing the SHA to the commit that tag points to (API: `GET /repos/{owner}/{repo}/commits/{tag}`). Dependabot `github-actions` updates may still apply depending on `dependabot.yml` limits
