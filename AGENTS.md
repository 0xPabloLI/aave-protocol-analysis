...
- Prefer keeping transient budget-parse fields off the shipped Brevis campaign shape: gRPC can attach `budgetNormalizedAmount` / `budgetTokenSymbol` on each `BrevisCampaignItem` until `fetchBrevisAprs` computes `totalBudget`, then `stripBrevisBudgetParseFields` removes them so runtime/API/CSV stay slim
