...
- Merkl forecast uses **cron-write/API-read-only** pattern: cron (every 10m) refreshes `snapshotCache`; API requests never call Merkl API
- CoinGecko endpoints use same pattern: cron warms cache, API only reads
- **Startup warmup**: Cron schedules don't run immediately on startup, so explicit warmup calls in `server.ts` pre-populate caches before `app.listen()`
