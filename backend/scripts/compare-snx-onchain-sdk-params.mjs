...
async function main() {
  ...
  console.log(
    "Fetching on-chain reserves (getReservesHumanized) for AaveV3Ethereum..."
  );
  ...
  const onchainReserve = reserves.find(
    (r) =>
      String(r.underlyingAsset || "").toLowerCase() === SNX_TOKEN.toLowerCase()
  );
  if (!onchainReserve) {
  ...
  const onchain = {
    ...
    optimalUsageRate:
      onchainReserve.optimalUsageRate ?? onchainReserve.optimalUsageRatio,
    availableLiquidity: onchainReserve.availableLiquidity,
    totalVariableDebt:
      onchainReserve.totalVariableDebt ??
      onchainReserve.totalScaledVariableDebt,
  ...
  const deficitRaw =
    onchainReserve.deficit != null ? String(onchainReserve.deficit) : null;
  ...
  const ocLiqRaw =
    onchain.availableLiquidity != null
      ? String(onchain.availableLiquidity)
      : null;
  const ocDebtRaw =
    onchain.totalVariableDebt != null && onchain.variableBorrowIndex != null
      ? (BigInt(String(onchain.totalVariableDebt)) *
          BigInt(String(onchain.variableBorrowIndex))) /
        RAY
  ...
  const sdkLiq =
    sdk.availableLiquidity != null ? String(sdk.availableLiquidity) : null;
  const sdkDebt =
    sdk.totalVariableDebt != null ? String(sdk.totalVariableDebt) : null;
  ...
  console.log(
    "    一致:",
    ocLiqRaw !== null && sdkLiq !== null && ocLiqRaw === sdkLiq
      ? "✓"
      : ocLiqRaw === null || sdkLiq === null
        ? "—"
        : "✗"
  );
  console.log(
    "  totalVariableDebt (raw, 链上若为 scaled 则已按 variableBorrowIndex 换算):"
  );
  console.log(
    "    链上:",
    ocDebtRaw != null
      ? typeof ocDebtRaw === "string"
        ? ocDebtRaw
        : String(ocDebtRaw)
      : "—"
  );
  ...
  console.log(
    "    一致:",
    ocDebtRaw !== null && sdkDebt !== null && String(ocDebtRaw) === sdkDebt
      ? "✓"
      : ocDebtRaw == null || sdkDebt == null
        ? "—"
        : "✗"
  );
  const ocSum =
    ocLiqBig != null && ocDebtForSum != null ? ocLiqBig + ocDebtForSum : null;
  const sdkSum =
    sdkLiqBig != null && sdkDebtBig != null ? sdkLiqBig + sdkDebtBig : null;
  ...
  console.log(
    "    一致:",
    ocSum !== null && sdkSum !== null && ocSum === sdkSum
      ? "✓"
      : ocSum === null || sdkSum === null
        ? "—"
        : "✗"
  );
  ...
  console.log(
    "  On-chain (%):",
    fmtPct(rayToPct(onchain.baseVariableBorrowRate))
  );
  ...
  for (const [label, onchainKey, unit] of keys) {
    ...
    const match =
      oc !== undefined &&
      sd !== undefined &&
      String(oc).trim() === String(sd).trim();
    console.log(`  ${label}:`);
    console.log(
      "    On-chain:",
      fmt(oc),
      unit === "RAY" && oc != null ? `(${fmtPct(rayToPct(oc))})` : ""
    );
    console.log(
      "    SDK:    ",
      fmt(sd),
      unit === "RAY" && sd != null ? `(${fmtPct(rayToPct(sd))})` : ""
    );
  ...
  const ocLiq =
    onchain.availableLiquidity != null
      ? BigInt(String(onchain.availableLiquidity))
      : null;
  const ocDebt =
    onchain.totalVariableDebt != null && onchain.variableBorrowIndex != null
      ? (BigInt(String(onchain.totalVariableDebt)) *
  ...
  const ocUtilPct =
    ocLiq != null && ocDebt != null && ocLiq + ocDebt > 0n
      ...
      : null;
  console.log(
    "    On-chain (availableLiquidity + totalVariableDebt → util%):",
    ocUtilPct != null ? `${ocUtilPct.toFixed(4)}%` : "—"
  );
  console.log(
    "    SDK utilizationPct:",
    sdk.utilizationPct != null ? `${sdk.utilizationPct}%` : "—"
  );
  console.log(
    "    Match:",
    ocUtilPct != null &&
      sdk.utilizationPct != null &&
      Math.abs(ocUtilPct - sdk.utilizationPct) < 0.01
      ? "✓"
      : "✗"
  );
  ...
  console.log(
    "    SDK borrowApy:",
    sdk.borrowApy != null ? `${sdk.borrowApy}%` : "—"
  );

  console.log("\n--- 结论 ---");
  const slope1Match =
    fmt(onchain.variableRateSlope1) === fmt(sdk.variableRateSlope1);
  const slope2Match =
    fmt(onchain.variableRateSlope2) === fmt(sdk.variableRateSlope2);
  const optimalMatch =
    fmt(onchain.optimalUsageRate) === fmt(sdk.optimalUsageRate);
  const utilMatch =
    ocUtilPct != null &&
    sdk.utilizationPct != null &&
    Math.abs(ocUtilPct - sdk.utilizationPct) < 0.05;
