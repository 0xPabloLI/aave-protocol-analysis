...
const meritCampaignBreakdown = {
  ...
  properties: {
    ...
    positionCapNative:  { type: 'string', description: 'Per-user position cap as native raw amount string (Merkl/Brevis). Mutually exclusive with positionCapUsd.' },
    positionCapUsd:     { type: 'number', description: 'Per-user position cap in USD (Merit). Mutually exclusive with positionCapNative.' },
...
const merklBreakdown = {
  ...
  properties: {
    ...
    positionCapNative:  { type: 'string', description: 'Per-user position cap as native raw amount string (from computeMethod=maxDeposit campaigns). Mutually exclusive with positionCapUsd.' },
    positionCapUsd:     { type: 'number', description: 'Per-user position cap in USD (Merit). Mutually exclusive with positionCapNative.' },
    isCombineCap:        { type: 'boolean', description: 'Whether the position cap is shared across supply+borrow sides' },
    lastEndedCampaign: {
      type: 'object',
...
const brevisBreakdown = {
  ...
  properties: {
    ...
    totalBudget:        { type: 'number' },
    latestTvl:          { type: 'number' },
    positionCapUsd:     { type: 'number', description: 'Per-user position cap in USD (from MetaMask description text). Mutually exclusive with positionCapNative.' },
    isCombineCap:       { type: 'boolean', description: 'Whether the position cap is shared across supply+borrow sides' },
  },
