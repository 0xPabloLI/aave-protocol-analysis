...
import {
  serializeReserveForApi,
  roundTo6,
} from "../src/services/marketsApiSerialize.js";
...
test("serializeReserveForApi falls back to MAX_APR for TARGET_TOTAL_APR without budgetBoundMode", () => {
  const reserve: RuntimeReserveData = {
  ...
  const bd = result.merklSupplys![0].breakdowns[0];
  assert.equal(bd.campaignType, "TARGET_TOTAL_APR");
  assert.equal("plannedDaily" in bd, true);
});

test("serializeReserveForApi passes through aaveProReserveId for V4 reserves", () => {
  const reserve: RuntimeReserveData = {
    reserveId:
      "1:0x973a023a77420ba610f06b3858ad991df6d85a01:0x973a023a77420ba610f06b3858ad991df6d85a02:0xcca852bc40e560adc3b1cc58ca5b55638ce826c9",
    ...
    aaveProReserveId:
      "MTo6MHg5NzNhMDIzQTc3NDIwYmE2MTBmMDZiMzg1OGFEOTkxRGY2ZDg1QTA4Ojo0",
  };

  const api = serializeReserveForApi(reserve);

  assert.equal(
    api.aaveProReserveId,
    "MTo6MHg5NzNhMDIzQTc3NDIwYmE2MTBmMDZiMzg1OGFEOTkxRGY2ZDg1QTA4Ojo0"
  );
});
...
test("serializeReserveForApi omits aaveProReserveId when empty string", () => {
  const reserve: RuntimeReserveData = {
    reserveId:
      "1:0xdefdefdefdefdefdefdefdefdefdefdefdefdef1:0xdefdefdefdefdefdefdefdefdefdefdefdefdef2:0x06002e9c4412cb7814a791ea3666d905871e536a",
    marketName: "AaveV4Ethereum",
    chainName: "Ethereum",
    chainId: 1,
    tokenName: "Test",
...
test("serializeReserveForApi passes through isFrozen and isPaused when true", () => {
  ...
  const paused: RuntimeReserveData = {
    ...frozen,
    isFrozen: undefined,
    isPaused: true,
  };
...
test("serializeReserveForApi outputs isActive:false for inactive V4 reserve", () => {
  const reserve: RuntimeReserveData = {
    reserveId:
      "1:0x1111111111111111111111111111111111111111:0xinactive0000000000000000000000000000000000:0x06002e9c4412cb7814a791ea3666d905871e536a",
    marketName: "AaveV4Ethereum",
    chainName: "Ethereum",
    chainId: 1,
    tokenName: "Inactive",
...
test("serializeReserveForApi does NOT output isActive when true or absent", () => {
  const reserve: RuntimeReserveData = {
    reserveId:
      "1:0x2222222222222222222222222222222222222222:0xactive000000000000000000000000000000000000:0xcca852bc40e560adc3b1cc58ca5b55638ce826c9",
    marketName: "AaveV4Ethereum",
    chainName: "Ethereum",
    chainId: 1,
    tokenName: "Active",
...
test("serializeReserveForApi output covers all EXPECTED_RUNTIME_FIELDS", () => {
  ...
  assert.equal(
    missing.length,
    0,
    `serializeReserveForApi is missing fields: ${missing.join(", ")}`
  );
});
...
// AAV-1222: ltv and liquidationThreshold serialization
test("serializeReserveForApi outputs ltv with roundTo6", () => {
  const reserve = makeFullReserve();
  reserve.ltv = 80.1234567;
  const api = serializeReserveForApi(reserve);
  assert.equal(api.ltv, 80.123457);
});

test("serializeReserveForApi outputs liquidationThreshold with roundTo6", () => {
  const reserve = makeFullReserve();
  reserve.liquidationThreshold = 82.5000001;
  const api = serializeReserveForApi(reserve);
  assert.equal(api.liquidationThreshold, 82.5);
});

test("serializeReserveForApi omits ltv and liquidationThreshold when undefined", () => {
  const reserve: RuntimeReserveData = {
    reserveId: "AaveV3Ethereum:1:0xabc",
    marketName: "AaveV3Ethereum",
    chainName: "Ethereum",
  ...
  const api = serializeReserveForApi(reserve);
  assert.equal("ltv" in api, false);
  assert.equal("liquidationThreshold" in api, false);
});
...
test("serializeReserveForApi computes incentive APR for TARGET_TOTAL_APR supply breakdown", () => {
  ...
  assert.equal(breakdown?.campaignType, "TARGET_TOTAL_APR");
  assert.ok(
    breakdown!.campaignApr < 7.7,
    `campaignApr (${breakdown!.campaignApr}) should be less than targetAPR (7.7)`
  );
  assert.ok(
    breakdown!.campaignApr > 0,
    `campaignApr (${breakdown!.campaignApr}) should be positive`
  );
...
test("serializeReserveForApi computes incentive APR for TARGET_TOTAL_APR borrow breakdown", () => {
  ...
  assert.ok(
    breakdown!.campaignApr > 0,
    "Borrow incentive APR should be positive when targetAPR < nativeBorrowAPY"
  );
...
test("serializeReserveForApi returns 0 incentive APR for TARGET_TOTAL_APR when nativeAPY already exceeds target", () => {
  ...
  assert.equal(
    breakdown?.campaignApr,
    0,
    "Incentive APR should be 0 when native exceeds target"
  );
});
...
test("serializeReserveForApi preserves raw campaignApr for non-TARGET_TOTAL_APR breakdowns", () => {
  ...
  assert.equal(
    breakdown?.campaignApr,
    5,
    "Non-TARGET_TOTAL_APR should use raw campaignApr × 100"
  );
});
...
test("serializeReserveForApi falls back to raw campaignApr for TARGET_TOTAL_APR when supplyApy is undefined", () => {
  ...
  assert.equal(
    breakdown?.campaignApr,
    4.7,
    "Without supplyApy, TARGET_TOTAL_APR should fall back to raw campaignApr × 100"
  );
});
