...
import {
  getFetchResultOrDefault,
  mergeWithPartialStale,
  correctFetchResult,
} from "../src/services/marketsService.js";
...
function baseInput(
  overrides: Partial<PartialStaleMergeInput>
): PartialStaleMergeInput {
  return {
    freshData: [],
    v3Succeeded: true,
    v4Succeeded: true,
...
test("both succeed → full merged dataset, both fetchedAt updated", () => {
  ...
  const result = mergeWithPartialStale(
    baseInput({
...
test("V4 RPC fallback → stale cache survives multiple cycles", () => {
  ...
  const result1 = mergeWithPartialStale(
    baseInput({
      freshData: [rpcV4],
      v3Succeeded: true,
      v4Succeeded: true,
  ...
  const result2 = mergeWithPartialStale(
    baseInput({
      freshData: [rpcV4],
      v3Succeeded: true,
      v4Succeeded: true,
