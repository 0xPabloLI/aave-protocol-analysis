...
test("buildSnapshotRow: supply_incentives_apr position is null (column removed)", () => {
  ...
  assert.ok(
    !MARKET_COLUMNS.includes(
      "supply_incentives_apr" as (typeof MARKET_COLUMNS)[number]
    )
  );
  assert.ok(
    !MARKET_COLUMNS.includes(
      "borrow_incentives_apr" as (typeof MARKET_COLUMNS)[number]
    )
  );
});
...
test("buildSnapshotRow: incentive_details is per-campaign structure (JSON parseable)", () => {
  ...
  assert.ok(parsed.merklSupplys);
  assert.equal(parsed.merklSupplys[0].link, "https://merkl.com/r1");
  assert.equal(parsed.merklSupplys[0].breakdowns[0].apr, 0.02);
  assert.equal(parsed.merklSupplys[0].breakdowns[0].key, "test-cmp");
  assert.equal(parsed.merklSupplys[0].breakdowns[0].endDate, "2025-12-31");
});
...
test("buildConfigRow: returns { row, hash } with hash as last element of row", () => {
  ...
  assert.equal(
    row[row.length - 1],
    hash,
    "hash must be the last element (content_hash column)"
  );
});
...
test("buildConfigRow: hash excludes snapshot_ts (only content fields)", () => {
  ...
  assert.equal(
    a.hash,
    b.hash,
    "same config content must produce same hash regardless of ts"
  );
});
...
test("buildConfigRow: row length matches MARKET_CONFIG_COLUMNS count", () => {
  ...
  assert.equal(
    row.length,
    MARKET_CONFIG_COLUMNS.length,
    `expected ${MARKET_CONFIG_COLUMNS.length} columns (including content_hash)`
  );
});
...
test("warmConfigHashes: DISTINCT ON picks latest per reserve_id (mocked)", () => {
  ...
  for (const r of rows) {
    ...
    if (
      ...
      r.snapshot_ts >
        (rows.find(
          (x) => x.reserve_id === r.reserve_id && x.content_hash === existing
        )?.snapshot_ts ?? "")
    ) {
...
test("warmConfigHashes: when persistence disabled, returns 0 without DB call", async () => {
  ...
  const { warmConfigHashes } =
    await import("../src/services/persistenceService.js");
...
test("buildConfigRow: content field count = columns - 2 (minus snapshot_ts and content_hash)", () => {
  ...
  assert.equal(
    ...
    MARKET_CONFIG_COLUMNS.length - 2,
    "content fields should be columns minus snapshot_ts and content_hash"
  );
  const recomputedHash = computeHash(contentFields);
  assert.equal(
    hash,
    recomputedHash,
    "hash should match recomputed hash from content fields"
  );
});
...
test("oraclePriceHashes key format uses pipe delimiter (not colon like reserveId)", () => {
  ...
  assert.ok(
    !reserveIdFormat.includes("|"),
    "reserveId does NOT use pipe delimiter"
  );
  assert.ok(oracleKeyFormat.includes("|"), "oracle key uses pipe delimiter");
  assert.ok(
    !oracleKeyFormat.includes(":"),
    "oracle key does NOT use colon delimiter"
  );
  assert.notDeepEqual(
...
test("shrinkOraclePriceHashes: only keys in current persist batch survive", () => {
  const currentKeys = new Set([
    oraclePriceKey(1, "0xaaa", "cfg1"),
    oraclePriceKey(1, "0xbbb", "cfg2"),
  ]);
  const staleKeys = [
    oraclePriceKey(1, "0xold", "cfg0"),
    oraclePriceKey(999, "0xstale", "cfg9"),
  ];
