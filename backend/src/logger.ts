...
/**
 * Deep-scrub sensitive keys from log metadata (log_scrubbing gate).
...
const scrubFormat = winston.format((info) => {
  if (typeof info === "object" && info !== null) {
    const { level, message, timestamp, ...meta } = info as Record<
      ...
      unknown
    >;
    const scrubbed = scrubSensitiveMeta(meta);
    return { ...scrubbed, level, message, timestamp } as typeof info;
  }
  return info;
})();
