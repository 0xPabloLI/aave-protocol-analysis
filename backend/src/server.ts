...
} from "./services/onchainDataService.js";
import {
  refreshOracleCache,
  getOracleCacheStats,
...
try {
  ...
  logger.warn(
    "Failed to set undici globalDispatcher (non-fatal, using defaults):",
...
const PORT: number = (() => {
  ...
  if (Number.isNaN(parsed)) {
    logger.warn(
      `⚠️  Invalid PORT value "${process.env.PORT}", using default 3001`
    );
    return 3001;
  }
  ...
  if (parsed !== 0 && (parsed < 1 || parsed > 65535)) {
    logger.warn(
      `⚠️  PORT value ${parsed} is out of valid range (0 or 1-65535), using default 3001`
    );
    return 3001;
  }
  return parsed;
...
/** Collect process memory metrics for /health endpoint and external monitoring. */
function getMemoryMetrics() {
  ...
  return {
    ...
    externalMB: Math.round(heapStats.external_memory / MB),
    oldSpace: oldSpace
      ? {
          usedMB: Math.round(oldSpace.space_used_size / MB),
          sizeMB: Math.round(oldSpace.space_size / MB),
        }
      : undefined,
...
const healthHandler = (_req: express.Request, res: express.Response) => {
  ...
  if (!marketsReady || marketsStale) {
    res.status(503).json({
      ...
      rpc: {
        summary: rpcHealth.summary,
        ...(chainsWithAllSuppressed.length > 0
          ? { chainsWithAllSuppressed }
          : {}),
      },
...
function findChainsWithAllSuppressed(rpcHealth: {
  endpoints: Array<{ chainId: number; status: string }>;
}): number[] {
...
app.use((req, res) => {
  ...
  const ua = sanitizeForLog(req.headers["user-agent"] ?? "-").slice(
    0,
    MAX_UA_LEN
  );
  (logger.info as (meta: object, msg: string) => void)(
    { method, path, ip, ua },
    "404"
  );
  res.status(404).json({
...
if (isPersistenceEnabled()) {
  ...
  try {
    ...
    logger.error(
      "❌ Auto-migration failed — starting server anyway (DB may be unreachable). Background retry every 60s:",
      error
    );
    ...
    const migrationRetryTimer = setInterval(async () => {
      ...
      if (++retryCount > MAX_RETRIES) {
        logger.error(
          "🔄 Migration retry exhausted after 1h — giving up. Restart the server to retry."
        );
        clearInterval(migrationRetryTimer);
        return;
      }
      try {
        logger.info(
          `🔄 Background migration retry #${retryCount} — acquiring DB pool...`
        );
        await runMigrationWithWarmup();
        ...
        logger.warn(
          `⚠️  Background migration retry #${retryCount} failed (will retry in 60s):`,
          error
        );
      }
    }, 60_000).unref();
...
const RSS_RESTART_THRESHOLD_MB =
  Number.parseInt(process.env.RSS_RESTART_THRESHOLD_MB ?? "", 10) || 0;
...
setInterval(() => {
  ...
  const undiciSummary =
    Object.entries(
      undiciStats as Record<
        string,
        { connected?: number; free?: number; running?: number; queued?: number }
      >
    )
      .map(
        ([origin, s]) =>
          `${new URL(origin).hostname}=${s.connected ?? 0}/${s.free ?? 0}/${s.running ?? 0}`
      )
      .join(" ") || "none";
  ...
  const spaceSummary = heapSpaces
    ...
    .map(
      (s) =>
        `${s.space_name.replace(/_space$/, "")}=${Math.round(s.space_used_size / 1024 / 1024)}/${Math.round(s.space_size / 1024 / 1024)}`
    )
    .join(" ");
  logger.info(
  ...
  if (
    RSS_RESTART_THRESHOLD_MB > 0 &&
    mem.rss > RSS_RESTART_THRESHOLD_MB * 1024 * 1024
  ) {
    logger.error(
...
logger.info(
  "🔄 Starting cache warmup (server already listening, health returns 503 until warm)..."
);
...
Promise.allSettled([
  refreshOnchainCache()
    .then(() =>
      logger.info("✅ On-chain cache (deficit, baseRate) warmed on startup")
    )
    .catch((error) =>
      logger.warn("⚠️  Failed to warm on-chain cache on startup:", error)
    ),
  refreshOracleCache()
    .then(() => logger.info("✅ Oracle price cache warmed on startup"))
    .catch((error) =>
      logger.warn("⚠️  Failed to warm oracle cache on startup:", error)
    ),
  warmMarketsCache()
    .then(() => logger.info("✅ Markets cache warmed on startup"))
    .catch((error) =>
      logger.warn("⚠️  Failed to warm markets on startup:", error)
    ),
  warmCoingeckoCategoriesCache()
  ...
  warmCoingeckoFdvCache()
    ...
    .catch((error) =>
      logger.warn("⚠️  Failed to warm coingecko FDV on startup:", error)
    ),
])
  .then(async () => {
    ...
    try {
      ...
      logger.info(
        `✅ Forecast snapshot cache warmed on startup: requested=${summary.requested}, fulfilled=${summary.fulfilled}, failed=${summary.failed}`
      );
    } catch (error) {
