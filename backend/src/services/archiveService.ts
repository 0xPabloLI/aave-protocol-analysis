...
async function hasPendingArchiveJob(): Promise<{
  id: number;
...
async function hasRunningArchiveJob(): Promise<{
  id: number;
  workflowRunId: number;
} | null> {
  const pool = getPool();
...
async function updateArchiveJob(
  jobId: number,
...
async function triggerArchiveWorkflow(): Promise<number> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `GitHub workflow_dispatch failed (${response.status}): ${body}`
    );
  }
...
async function checkWorkflowRunStatus(
  ...
  if (!response.ok)
    throw new Error(
      `Failed to check workflow run ${runId} (${response.status})`
    );
  const data = (await response.json()) as {
    status: string;
...
async function cleanupPostgres(retainDays: number): Promise<void> {
  ...
  const tables = [
    ...
    { table: "side_data_snapshots", timeCol: "created_at" },
  ];

  for (const { table, timeCol } of tables) {
    logger.info(
      `Cleaning ${table}: deleting rows older than ${retainDays} days`
    );
...
export async function runArchiveCheck(): Promise<ArchiveCheckResult> {
  ...
  if (!getGithubToken()) {
    logger.warn(
      "DB size exceeds threshold but GITHUB_ACTIONS_TOKEN not configured; skipping archive"
    );
  ...
  if (runningJob) {
    ...
    if (runStatus.conclusion === "success") {
      try {
        ...
        await updateArchiveJob(runningJob.id, {
          status: "succeeded",
  ...
  try {
    ...
    await updateArchiveJob(jobId, {
      status: "running",
      workflow_run_id: workflowRunId,
    });
    logger.info(
      `Triggered archive workflow (run ${workflowRunId}), job ${jobId}`
    );
    return {
      action: "triggered",
      pgSizeBytes,
