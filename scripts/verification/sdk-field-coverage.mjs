...
const SDK_FIELD_CHECKS = [
  ...
  {
    key: "borrowInfo.utilizationRate.value",
    get: (r) => r.borrowInfo?.utilizationRate?.value,
  },
  { key: "aToken.address", get: (r) => r.aToken?.address },
  { key: "vToken.address", get: (r) => r.vToken?.address },
  {
    key: "supplyInfo.supplyCap.amount.value",
    get: (r) => r.supplyInfo?.supplyCap?.amount?.value,
  },
  { key: "supplyInfo.supplyCap.usd", get: (r) => r.supplyInfo?.supplyCap?.usd },
  { key: "supplyInfo.apy.value", get: (r) => r.supplyInfo?.apy?.value },
  {
    key: "borrowInfo.borrowingState",
    get: (r) => r.borrowInfo?.borrowingState,
  },
  {
    key: "borrowInfo.borrowCap.amount.value",
    get: (r) => r.borrowInfo?.borrowCap?.amount?.value,
  },
  { key: "borrowInfo.borrowCap.usd", get: (r) => r.borrowInfo?.borrowCap?.usd },
  { key: "borrowInfo.apy.value", get: (r) => r.borrowInfo?.apy?.value },
  {
    key: "borrowInfo.availableLiquidity.amount.raw",
    get: (r) => r.borrowInfo?.availableLiquidity?.amount?.raw,
  },
  {
    key: "borrowInfo.total.amount.raw",
    get: (r) => r.borrowInfo?.total?.amount?.raw,
  },
  {
    key: "borrowInfo.reserveFactor.raw",
    get: (r) => r.borrowInfo?.reserveFactor?.raw,
  },
  {
    key: "borrowInfo.variableRateSlope1.raw",
    get: (r) => r.borrowInfo?.variableRateSlope1?.raw,
  },
  {
    key: "borrowInfo.variableRateSlope2.raw",
    get: (r) => r.borrowInfo?.variableRateSlope2?.raw,
  },
  {
    key: "borrowInfo.optimalUsageRate.raw",
    get: (r) => r.borrowInfo?.optimalUsageRate?.raw,
  },
...
function run() {
  ...
  try {
    ...
    console.error(
      "缺少 data/debug/aave-all-markets-data.json，请先运行 fetcher 生成（如 npm run dev）"
    );
    process.exit(1);
  }
  ...
  console.log(
    `(已排除 isFrozen/isPaused: ${excludedFrozenPaused} 条，与 fetchMarketsPayload 的 payload.data 条数应对齐，约 240)`
  );
  ...
  results.forEach((r) => {
    console.log(
      `  ${r.reserveId} (${r.symbol}) missing: ${r.missing.join(", ")}`
    );
  });
}
