const FIELD_RULES = [
  ...
  },
  {
    ...
    tolerance: 0.01,
    unit: "percent",
    description: "Collateral Factor (V4)",
  },
];
...
function rawToHumanReadable(rawStr, decimals) {
  ...
  try {
    ...
    return (
      parseFloat(whole.toString()) +
      parseFloat(frac.toString()) / parseFloat(divisor.toString())
    );
  } catch {
    return null;
  }
...
function diffField(
  ...
  let bNum =
    typeof backendVal === "string" ? parseFloat(backendVal) : backendVal;
  const uNum =
    typeof aaveUiVal === "string" ? parseFloat(aaveUiVal) : aaveUiVal;
  ...
  if (absTolerance > 0 || relTolerance > 0) {
    return {
      ...
      aaveUi: uNum,
      diff,
      tolerance: Math.max(absTolerance, relTolerance),
...
function compareReserves(backendReserves, aaveUiReserves, version) {
  ...
  const backendByKey = new Map(
    relevantBackend.map((r) => [buildKey(r, version), r])
  );
...
function generateReport(v3Result, v4Result, backendSource, aaveUiSources) {
  ...
  const outOfTolerance = allMismatches.filter(
    (m) => m.status === "out_of_tolerance"
  );
  const withinTolerance = allMismatches.filter(
    (m) => m.status === "within_tolerance"
  );
  ...
  return {
    ...
    summary: {
      ...
      missingInBackend: {
        v3: v3Result.missingInBackend.length,
        v4: v4Result.missingInBackend.length,
      },
      missingInAaveUi: {
        v3: v3Result.missingInAaveUi.length,
        v4: v4Result.missingInAaveUi.length,
      },
      fieldMismatches: {
...
function printSummary(report) {
  ...
  console.log(
    `Aave UI: V3=${report.aaveUiSources.v3}, V4=${report.aaveUiSources.v4}\n`
  );

  console.log("--- Reserve Matching ---");
  console.log(
    `  V3: ${s.matched.v3} matched, ${s.missingInBackend.v3} in Aave UI only, ${s.missingInAaveUi.v3} in Backend only`
  );
  console.log(
    `  V4: ${s.matched.v4} matched, ${s.missingInBackend.v4} in Aave UI only, ${s.missingInAaveUi.v4} in Backend only`
  );
  ...
  console.log(
    `  Exact match:       ${s.fieldMismatches.exactMatchFields} field-reserve pairs`
  );
  ...
  if (report.outOfTolerance.length > 0) {
    ...
    for (const m of report.outOfTolerance.slice(0, 20)) {
      ...
      console.log(
        `  ${label} | ${m.field}: backend=${m.backend}, aaveUi=${m.aaveUi}, diff=${m.diff?.toFixed(4)}, tolerance=${m.tolerance}`
      );
    }
  ...
  if (
    report.v3MissingInBackend.length > 0 ||
    report.v4MissingInBackend.length > 0
  ) {
    console.log("\n--- In Aave UI but NOT in Backend ---");
    for (const m of [
      ...report.v3MissingInBackend,
      ...report.v4MissingInBackend,
    ].slice(0, 20)) {
      console.log(
        `  ${m.tokenSymbol ?? m.key} (chain ${m.chainId ?? m.spokeChainId})`
      );
    }
  }

  if (
    report.v3MissingInAaveUi.length > 0 ||
    report.v4MissingInAaveUi.length > 0
  ) {
    console.log("\n--- In Backend but NOT in Aave UI ---");
    for (const m of [
      ...report.v3MissingInAaveUi,
      ...report.v4MissingInAaveUi,
    ].slice(0, 20)) {
  ...
  const hasIssues =
    s.fieldMismatches.outOfTolerance > 0 ||
    s.fieldMismatches.booleanMismatch > 0;
