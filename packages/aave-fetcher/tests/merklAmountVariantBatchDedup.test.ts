...
import { normalizeCampaignType } from "@internal/aave-shared-contracts";
...
test("normalizeCampaignType: resolves all AMOUNT variants from distributionType", () => {
  assert.equal(
...
test("batch dedup: same token across multiple campaigns resolves once", () => {
  const entries = [
    {
      ...
      rewardSym: "TOKEN_A",
      campaignType:
        "FIX_REWARD_AMOUNT_PER_LIQUIDITY_VALUE" as ForecastCampaignTypeLite,
    },
    {
      campaignId: "c2",
      chainId: 1,
    ...
    },
    {
      ...
      rewardSym: "TOKEN_B",
      campaignType:
        "FIX_REWARD_AMOUNT_PER_LIQUIDITY_AMOUNT" as ForecastCampaignTypeLite,
    },
  ];
  ...
  for (const entry of entries) {
    tokensToResolve.add(
      `${entry.chainId}:${entry.rewardAddr}:${entry.rewardSym}` as PriceLookupKey
    );
  }
...
test("batch dedup: AMOUNT_PER_AMOUNT entries collect both reward and target tokens", () => {
  ...
  for (const entry of entries) {
    ...
    if (
      ...
      tokensToResolve.add(
        `${entry.chainId}:${entry.targetAddr}:${entry.targetSym}` as PriceLookupKey
      );
    }
  }
...
test("campaign.distributionType used over opp.distributionType when opp is empty", () => {
  ...
  const fromOpp = normalizeCampaignType({
    distributionType: oppDistributionType,
  });
  const fromCampaign = normalizeCampaignType({
