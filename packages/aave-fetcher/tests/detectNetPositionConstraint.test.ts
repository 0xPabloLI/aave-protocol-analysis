...
const makeReserveIdSet = (reserveIds: string[]): Set<string> =>
  new Set(reserveIds);
...
describe("B4: detectNetPositionConstraint — four-layer detection", () => {
  it('L0 (type match) takes priority over L1 (looping) — AAVE_NET_LENDING with "looping" in name still returns constraint', async () => {
    ...
    const reserveIdSet = makeReserveIdSet([
      "1:0xpool:0xusdt",
      "1:0xpool:0xborrowtoken",
    ]);
    const symbolLookup = makeSymbolLookup([]);
    const result = await detectNetPositionConstraint(
      opp,
      "0xusdt",
      "1:0xpool:0xusdt",
      reserveIdSet,
  ...
  it('L1 excludes looping — name contains "looping required" returns null (non-NET type)', async () => {
    const opp: MerklOpportunityData = {
      ...
      description:
        "Ethena Liquid Leverage program. Users must also borrow USDT0.",
    };
  ...
  it('L1 excludes looping — description contains "looping" returns null (non-NET type)', async () => {
    const opp: MerklOpportunityData = {
      ...
      description:
        "This is a looping strategy where users supply and borrow the same asset.",
    };
    const reserveIdSet = makeReserveIdSet(["1:0xpool:0xusde"]);
    const symbolLookup = makeSymbolLookup([]);
    const result = await detectNetPositionConstraint(
  ...
  it("Layer 2 returns cached constraint when Layer 1 returns null", async () => {
    ...
    const cached: NetPositionConstraint = {
      sourceSide: "supply",
  ...
  it("Layer 3 LLM fallback when Layer 1 null + no cache", async () => {
    ...
    assert.deepEqual(result, {
      sourceSide: "supply",
      offsetReserveIds: ["1:0xpool:0xusdtb"],
    });
  });
  ...
  it("L0 distributionType match — AAVE_V4_NET_APR recognized as net position", async () => {
    ...
    assert.equal(result!.sourceSide, "supply");
    assert.ok(
      result!.offsetReserveIds.includes("1:0xspoke:0xusdg:Core"),
      "self token should be in offsetReserveIds"
    );
  });
  ...
  it("L0 distributionType match — AAVE_NET_APR recognized as net position (borrow side)", async () => {
    ...
    assert.ok(
      result!.offsetReserveIds.includes("1:0xpool:0xgho"),
      "self token should be in offsetReserveIds"
    );
  });
  ...
  it("L0 AAVE_NET_LENDING self-token always included in offsetReserveIds", async () => {
    ...
    assert.deepEqual(result, {
      sourceSide: "supply",
      offsetReserveIds: ["4326:0xpool:0xusdm"],
    });
  });
  ...
  it("Bug3: AAVE_NET_BORROWING includes self token as offset", async () => {
    ...
    assert.deepEqual(result, {
      sourceSide: "borrow",
      offsetReserveIds: ["1:0xpool:0xgho"],
    });
  });
  ...
  it("LLM offsetTokenSymbols not found → A2 skip (returns self-only constraint, no heuristic fallback)", async () => {
    // A2 behavior: unresolvable symbol is skipped + warned, not fail-fast.
    // Constraint still built with self reserveId (explicit add).
    const opp: MerklOpportunityData = {
      supply: [
        {
          campaignApr: 0.03,
  ...
  it("Bug5: V4 opp with reserve offsetLevel resolves exact match only", async () => {
    ...
    assert.deepEqual(result, {
      sourceSide: "supply",
      offsetReserveIds: [
        "1:0xv4spoke:0xsourceToken:Core",
        "1:0xv4spoke:0xrlusd:Core",
      ],
    });
  });
  ...
  it("LLM symbol not in symbolLookup → A2 skip (returns self-only constraint, no heuristic fallback)", async () => {
    const opp: MerklOpportunityData = {
      supply: [
        {
          campaignApr: 0.03,
  ...
  it("LLM resolves but offsetReserveIds empty → A2 skip (returns self-only constraint, no heuristic fallback)", async () => {
    const opp: MerklOpportunityData = {
      supply: [
        {
          campaignApr: 0.03,
...
describe("L0.5: composedNetPositionConstraint — 1-2 compute detection", () => {
  it("1-2 BORROW → returns net borrow with self reserveId", async () => {
    ...
    const reserveIdSet = makeReserveIdSet([
      "9745:0xpool:0xb8ce59fc3717ada4c02eadf9682a9e934f625ebb",
    ]);
    const result = composedNetPositionConstraint(
      opp,
      "9745:0xpool:0xb8ce59fc3717ada4c02eadf9682a9e934f625ebb",
      reserveIdSet
    );
  ...
  it("1-2 LEND → returns net supply with self reserveId", () => {
    ...
    const reserveIdSet = makeReserveIdSet([
      "1:0xpool:0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48",
    ]);
    const result = composedNetPositionConstraint(
      opp,
      "1:0xpool:0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48",
    ...
    assert.deepEqual(result, {
      sourceSide: "supply",
      offsetReserveIds: ["1:0xpool:0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48"],
    });
  });
...
describe("AAV-1206: LLM offset symbol normalization (CI + equiv groups + A2 skip + self reserveId)", () => {
  const equivLookup = buildEquivLookup();

  it("13. LLM returns case-variant offset → constraint resolved via CI (previously null)", async () => {
    const opp: MerklOpportunityData = {
      supply: [
        {
          campaignApr: 0.03,
      ...
      protocolVersion: "v3",
      opportunityType: "AAVE_SUPPLY",
      description: "net lending",
      offsetTokenAddresses: ["0xusde"],
    };
    const mockLlm = async (): Promise<LlmOutcome> => ({
      tag: "result",
      value: { sourceSide: "supply", offsetTokenSymbols: ["usde"] },
    });
    const reserveIdSet = makeReserveIdSet([
      "1:0xpool:0xusde",
      "1:0xpool:0xsource",
    ]);
    const symbolLookup = makeSymbolLookup([[1, "USDe", "0xusde"]]);
    const symbolLookupCI = buildSymbolLookupCI([
      { chainId: 1, tokenSymbol: "USDe", tokenAddress: "0xusde" },
    ]);
    const result = await detectNetPositionConstraint(
      opp,
      "0xsource",
    ...
    assert.deepEqual(result, {
      sourceSide: "supply",
      offsetReserveIds: ["1:0xpool:0xsource", "1:0xpool:0xusde"],
    });
  });

  it("14. LLM returns Unicode-variant offset → constraint resolved via equiv group", async () => {
    const opp: MerklOpportunityData = {
      supply: [
        {
          campaignApr: 0.03,
      ...
      opportunityType: "AAVE_SUPPLY",
      description: "net lending",
      offsetTokenAddresses: ["0xsource"],
    };
    const mockLlm = async (): Promise<LlmOutcome> => ({
      tag: "result",
      value: { sourceSide: "supply", offsetTokenSymbols: ["USDT"] },
    });
    const reserveIdSet = makeReserveIdSet([
      "42161:0xpool:0xusd0",
      "42161:0xpool:0xsource",
    ]);
    const symbolLookup = makeSymbolLookup([[42161, "USD₮0", "0xusd0"]]);
    const symbolLookupCI = buildSymbolLookupCI([
      { chainId: 42161, tokenSymbol: "USD₮0", tokenAddress: "0xusd0" },
    ]);
    const result = await detectNetPositionConstraint(
      opp,
      "0xsource",
    ...
    assert.deepEqual(result, {
      sourceSide: "supply",
      offsetReserveIds: ["42161:0xpool:0xsource", "42161:0xpool:0xusd0"],
    });
  });

  it("15. LLM returns unknown symbol → A2 skip, constraint still built with resolved offsets", async () => {
    const opp: MerklOpportunityData = {
      supply: [
        {
          campaignApr: 0.03,
    ...
    const mockLlm = async (): Promise<LlmOutcome> => ({
      tag: "result",
      value: { sourceSide: "supply", offsetTokenSymbols: ["USDe", "UNKNOWN"] },
    });
    const reserveIdSet = makeReserveIdSet([
      "1:0xpool:0xusde",
      "1:0xpool:0xsource",
  ...
  it("16. LLM returns all unknown symbols → constraint returns self only", async () => {
    const opp: MerklOpportunityData = {
      supply: [
        {
          campaignApr: 0.03,
    ...
    const mockLlm = async (): Promise<LlmOutcome> => ({
      ...
      value: {
        sourceSide: "supply",
        offsetTokenSymbols: ["UNKNOWN1", "UNKNOWN2"],
      },
    });
    const reserveIdSet = makeReserveIdSet(["1:0xpool:0xsource"]);
    const symbolLookup = makeSymbolLookup([]);
    const symbolLookupCI = buildSymbolLookupCI([]);
    const result = await detectNetPositionConstraint(
      opp,
    ...
    assert.deepEqual(result, {
      sourceSide: "supply",
      offsetReserveIds: ["1:0xpool:0xsource"],
    });
  });

  it("17. self reserveId explicit add — LLM omits source token → offsetReserveIds includes self", async () => {
    const opp: MerklOpportunityData = {
      supply: [
        {
          campaignApr: 0.03,
    ...
    assert.deepEqual(result, {
      sourceSide: "supply",
      offsetReserveIds: ["1:0xpool:0xgho", "1:0xpool:0xusde"],
    });
  });

  it("18. self reserveId idempotent — LLM includes source token → no duplication", async () => {
    const opp: MerklOpportunityData = {
      supply: [
        {
          campaignApr: 0.03,
    ...
    const mockLlm = async (): Promise<LlmOutcome> => ({
      tag: "result",
      value: { sourceSide: "supply", offsetTokenSymbols: ["GHO", "USDe"] },
    });
    const reserveIdSet = makeReserveIdSet([
      "1:0xpool:0xgho",
      "1:0xpool:0xusde",
    ...
    const symbolLookupCI = buildSymbolLookupCI([
      { chainId: 1, tokenSymbol: "GHO", tokenAddress: "0xgho" },
      { chainId: 1, tokenSymbol: "USDe", tokenAddress: "0xusde" },
    ]);
    const result = await detectNetPositionConstraint(
      opp,
