...
const debugPath = resolve(
  __dirname,
  "../../../data/debug/v4-raw-sdk-response.json"
);

test("V4 raw SDK data: reserveId fourth segment should be hubAddress (currently hubName - this test is RED)", (t) => {
  // data/ is gitignored (.gitignore) — the captured SDK response exists locally
  // only. Skip on fresh checkouts (CI) instead of failing on ENOENT.
  if (!existsSync(debugPath)) {
    t.skip(
      "data/debug/v4-raw-sdk-response.json not present (gitignored debug capture)"
    );
    return;
  }
  ...
  for (const r of reserves) {
    ...
    assert.ok(
      parts[3].startsWith("0x"),
      `V4 reserveId fourth segment should be hubAddress, got: ${parts[3]}`
    );
    assert.ok(
      parts[3].length === 42,
      `hubAddress should be 42 chars, got: ${parts[3].length}`
    );
  }
});
