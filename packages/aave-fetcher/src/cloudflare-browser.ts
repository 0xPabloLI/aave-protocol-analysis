...
const WORKER_DYNAMIC_MIN_INTERVAL_MS =
  cloudflareWorkerConfig.dynamicMinIntervalMs;
const WORKER_DYNAMIC_MAX_RETRIES = cloudflareWorkerConfig.dynamicMaxRetries;
const WORKER_DYNAMIC_CIRCUIT_BREAKER_MS =
  cloudflareWorkerConfig.dynamicCircuitBreakerMs;
...
logger.info(
  "🔒 Created Cloudflare Worker request semaphore with concurrency=1 (controls HTTP requests to Worker)"
);

function trimErrorSnippet(text: string): string {
  const cleaned = text.replace(/\s+/g, " ").trim();
  return cleaned.length > MAX_ERROR_SNIPPET
    ? `${cleaned.slice(0, MAX_ERROR_SNIPPET)}…`
    : cleaned;
}
...
export async function closeBrowserInstances(): Promise<{
  ...
  if (!CLOUDFLARE_WORKER_URL) {
    logger.warn(
      "⚠️ CLOUDFLARE_WORKER_URL not set, skipping browser instance close"
    );
    return { success: false, error: "CLOUDFLARE_WORKER_URL not set" };
  }

  logger.info(
    "🔌 Closing existing Cloudflare browser instances to release quota..."
  );
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text().catch(() => "");
      logger.error(
        `❌ Failed to close browser instances: HTTP ${response.status}: ${errorText}`
      );
      return { success: false, error: `HTTP ${response.status}: ${errorText}` };
    }
    ...
    if (data.closed > 0) {
      logger.info(
        `✅ Closed ${data.closed} browser instances, ${data.remaining} remaining`
      );
    } else {
      logger.info(
        `✅ No browser instances to close (${data.remaining} remaining)`
      );
    }
...
export async function checkAndReportSessionStatus(): Promise<void> {
  if (!CLOUDFLARE_WORKER_URL) {
    logger.warn(
      "⚠️ CLOUDFLARE_WORKER_URL not set, skipping session status check"
    );
    return;
  }
  ...
  if (stats.sessions && stats.sessions >= 3) {
    logger.warn(
      `⚠️ WARNING: ${stats.sessions} sessions exist, which is at or above Cloudflare's limit of 3 concurrent browsers!`
    );
...
async function scheduleDynamicSlot(): Promise<void> {
  ...
  try {
    ...
    if (Date.now() < workerDisabledUntil) {
      ...
      logger.info(
        `⏸️ scheduleDynamicSlot: Worker disabled, skipping rate limit wait (${Math.round(waitMs / 1000)}s remaining until re-enable)`
      );
      return;
    }
    ...
    const waitMs = Math.max(
      0,
      lastDynamicStartedAt + WORKER_DYNAMIC_MIN_INTERVAL_MS - now
    );
    if (waitMs > 0) {
      logger.info(
        `⏳ scheduleDynamicSlot: waiting ${Math.round(waitMs / 1000)}s before next Worker request (minInterval=${WORKER_DYNAMIC_MIN_INTERVAL_MS}ms)`
      );
      ...
      if (Date.now() < workerDisabledUntil) {
        ...
        logger.info(
          `⏸️ scheduleDynamicSlot: Worker disabled, skipping request (${Math.round(remaining / 1000)}s remaining until re-enable)`
        );
        return;
      }
    }
    ...
    logger.info(
      `🚀 scheduleDynamicSlot: starting Worker request #${totalWorkerRequests} (uptime=${Math.round(uptimeMs / 1000)}s, 429s=${totalWorker429s}累计, errors=${totalWorkerErrors})`
    );
  } finally {
    release();
  }
 ...
 */
export async function extractCampaignInfoWithWorker(
  key: string
): Promise<MeritCampaignInfo[]> {
  if (!CLOUDFLARE_WORKER_URL) {
    logger.warn(
      "⚠️ CLOUDFLARE_WORKER_URL not set, skipping Cloudflare Worker for campaign info"
    );
    return [];
  }
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text().catch(() => "");
      throw new Error(
        `Cloudflare Worker HTTP ${response.status}: ${errorText}`
      );
    }

    const data = (await response.json()) as {
      success: boolean;
      result?: MeritCampaignInfo[];
      error?: string;
    };
    ...
    if (!data.success) {
      logger.warn(
        `⚠️ Cloudflare Worker failed for ${key}:`,
 ...
 */
export async function extractMeritDynamicInfoWithWorker(
  key: string
): Promise<MeritDynamicInfo | null> {
  if (!CLOUDFLARE_WORKER_URL) {
    logger.warn(
      "⚠️ CLOUDFLARE_WORKER_URL not set, skipping Cloudflare Worker for dynamic merit info"
    );
    return null;
  }
 ...
 */
async function extractMeritDynamicInfoWithWorkerInternal(
  key: string,
  signal?: AbortSignal
): Promise<MeritDynamicInfo | null> {
  ...
  try {
    ...
    if (Date.now() < workerDisabledUntil) {
      const waitMs = workerDisabledUntil - Date.now();
      logger.info(
        `⏸️ Worker disabled until ${new Date(workerDisabledUntil).toISOString()} (${Math.round(waitMs / 1000)}s remaining), returning null for immediate fallback to Playwright`
      );
      return null;
    }
    ...
    for (let attempt = 0; attempt <= WORKER_DYNAMIC_MAX_RETRIES; attempt++) {
      ...
      if (Date.now() < workerDisabledUntil) {
        const waitMs = workerDisabledUntil - Date.now();
        logger.info(
          `⏸️ Worker disabled during retry attempt ${attempt + 1}, returning null for immediate fallback to Playwright`
        );
        return null;
      }
      ...
      if (Date.now() < workerDisabledUntil) {
        const waitMs = workerDisabledUntil - Date.now();
        logger.info(
          `⏸️ Worker disabled after scheduleDynamicSlot, returning null for immediate fallback to Playwright (${Math.round(waitMs / 1000)}s remaining)`
        );
        return null;
      }
      ...
      logger.debug(
        `🔒 [Worker Semaphore] Acquiring semaphore for Worker request (key: ${key})`
      );
      const release = await workerRequestSemaphore.acquire();
      logger.debug(
        `🔒 [Worker Semaphore] Acquired semaphore for Worker request (key: ${key})`
      );
      let released = false;
      ...
      try {
        ...
        if (Date.now() < workerDisabledUntil) {
          const waitMs = workerDisabledUntil - Date.now();
          logger.info(
            `⏸️ Worker disabled after acquiring semaphore, returning null for immediate fallback to Playwright (${Math.round(waitMs / 1000)}s remaining)`
          );
          safeRelease();
          return null;
        }
        ...
        if (response.status === 429) {
          ...
          const waitMs =
            retryAfter > 0 ? retryAfter * 1000 : WORKER_DYNAMIC_MIN_INTERVAL_MS;
          const errorText = await response.text().catch(() => "");
          ...
          logger.warn(
            `   • Raw error response: ${errorText ? trimErrorSnippet(errorText) : "No error text"}`
          );
          logger.warn(
            `   • Retry-After header: ${retryAfter}s (实际等待: ${retryAfter > 0 ? `${retryAfter}s` : `${Math.round(waitMs / 1000)}s (使用默认间隔)`})`
          );
          if (WORKER_DYNAMIC_FAIL_FAST) {
            logger.warn(
              `   • Fail-fast enabled: 立即返回 null，fallback 到 Playwright（不等待 ${Math.round(waitMs / 1000)}s）`
            );
          } else {
            logger.warn(
              `   • Will wait: ${Math.round(waitMs / 1000)}s before retry`
            );
          }
          logger.warn(
            `   • Total 429s so far: ${totalWorker429s} (累计 429 错误次数)`
          );
          logger.warn(
            `   • Current sessions: ${sessionStats.sessions ?? "unknown"} (limit: 3)`
          );
          if (sessionStats.sessionIds && sessionStats.sessionIds.length > 0) {
            logger.warn(
              `   • Session IDs: ${sessionStats.sessionIds.join(", ")}`
            );
          }
          logger.warn(
            `   • Worker stats: launches=${sessionStats.totalLaunches}, reuses=${sessionStats.totalReuses}`
          );

          workerDisabledUntil =
            Date.now() + Math.max(waitMs, WORKER_DYNAMIC_CIRCUIT_BREAKER_MS);
          // 触发所有等待中的 Promise，立即中断等待
        ...
        if (!response.ok) {
          ...
          const looksRateLimited =
            response.status === 500 &&
            /rate limit exceeded|code:\s*429|too many requests/i.test(
              errorText
            );

          const looksTransientPuppeteer =
            response.status === 500 &&
            /detached Frame|Execution context was destroyed|Target closed|Navigation failed|Protocol error/i.test(
              errorText
            );
          ...
          if (looksRateLimited && attempt < WORKER_DYNAMIC_MAX_RETRIES) {
            ...
            logger.warn(
              `🚨 Cloudflare Worker 500 (embedded rate limit) for ${key}:`
            );
            ...
            const backoffMs = Math.min(
              120000,
              WORKER_DYNAMIC_MIN_INTERVAL_MS * Math.pow(2, attempt)
            );
            logger.warn(
              `   • Will wait: ${Math.round(backoffMs / 1000)}s (exponential backoff)`
            );
            workerDisabledUntil =
              Date.now() +
              Math.max(backoffMs, WORKER_DYNAMIC_CIRCUIT_BREAKER_MS);
            // 触发所有等待中的 Promise，立即中断等待
            triggerWorkerDisabledResolvers();
          ...
          if (looksTransientPuppeteer && attempt < WORKER_DYNAMIC_MAX_RETRIES) {
            ...
            logger.warn(
              `⚠️ Cloudflare Worker transient puppeteer error for ${key}, retrying in ${backoffMs}ms`
            );
            safeRelease(); // Release semaphore before waiting
            await sleep(backoffMs);
            continue;
        ...
        const campaignInfo = Array.isArray(data.result.campaignInfo)
          ? data.result.campaignInfo
          : [];
    ...
    logger.warn(
      `⚠️ Cloudflare dynamic merit info Worker failed for ${key}:`,
      error
    );
    return null;
  }
