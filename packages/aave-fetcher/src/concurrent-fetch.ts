    ...
    if (market.supplyReserves && Array.isArray(market.supplyReserves)) {
      market.supplyReserves.forEach((reserve: any) => {
        ...
        const tokenPrice =
          ...
          undefined;
        const utilizationRaw = toFiniteNumber(
          reserve?.borrowInfo?.utilizationRate?.value
        );
        const utilizationPct =
          utilizationRaw !== null && utilizationRaw >= 0
            ? utilizationRaw * 100
            : undefined;
        ...
        const borrowCapIsOne =
          borrowCapValue !== undefined && toFiniteNumber(borrowCapValue) === 1;
        const isBorrowDisabled = hasProtocolReason
          ? false
          : isBorrowDisabledByState || borrowCapIsOne;
        ...
        const liquidity =
          reserve.borrowInfo?.availableLiquidity?.amount?.raw ?? undefined;
        const borrowed = reserve.borrowInfo?.total?.amount?.raw ?? undefined;
        const supplied = reserve.size?.amount?.raw ?? undefined;
        const supplyCap =
          reserve.supplyInfo?.supplyCap?.amount?.raw ?? undefined;
        const borrowCap =
          reserve.borrowInfo?.borrowCap?.amount?.raw ?? undefined;
        const protocolFee = percentValueToPercent(
          reserve.borrowInfo?.reserveFactor
        );
        const slopeBelowOptimal = percentValueToPercent(
          reserve.borrowInfo?.variableRateSlope1
        );
        const slopeAboveOptimal = percentValueToPercent(
          reserve.borrowInfo?.variableRateSlope2
        );
        const optimalUtilization = percentValueToPercent(
          reserve.borrowInfo?.optimalUsageRate
        );

        // AAV-1222: Collateral LTV and liquidation threshold from supplyInfo
        const ltv = percentValueToPercent(reserve.supplyInfo?.maxLTV);
        const liquidationThreshold = percentValueToPercent(
          reserve.supplyInfo?.liquidationThreshold
        );
...
export function buildMarketsBaseDataset(
  ...
  logger.info(
    `📊 Unified dataset: ${baseDataset.length} reserves (V3: ${v3Dataset.length}, V4: ${v4Dataset.length})`
  );
  return {
    baseDataset,
    v3Count: v3Dataset.length,
    v4Count: v4Dataset.length,
    v4Dataset,
    v4Raw,
    spokeHubTopology,
  };
...
export async function fetchV3MarketsWithTimeout(options: {
  ...
  const timeoutPromise = new Promise<never>((_, reject) => {
    timeoutId = setTimeout(() => {
      reject(
        new Error(`V3 fetch timeout (timeout after ${FETCH_TIMEOUT_MS}ms)`)
      );
    }, FETCH_TIMEOUT_MS);
  });
  return Promise.race([
...
export type V4RpcFallbackFn = () => Promise<{
...
export async function fetchV4ReservesWithTimeout(options: {
  ...
  try {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      timeoutId = setTimeout(() => {
        reject(
          new Error(`V4 fetch timeout (timeout after ${FETCH_TIMEOUT_MS}ms)`)
        );
      }, FETCH_TIMEOUT_MS);
    });
    ...
    logger.warn(
      `⚠️ [V4] Layer 1 SDK fetch failed: ${sdkError instanceof Error ? sdkError.message : String(sdkError)}`
    );
    sdkResult = null;
  }
    ...
    try {
      ...
      const rpcTimeout = new Promise<never>((_, reject) => {
        rpcTimeoutId = setTimeout(() => {
          reject(
            new Error(`V4 RPC fallback timeout (${RPC_FALLBACK_TIMEOUT_MS}ms)`)
          );
        }, RPC_FALLBACK_TIMEOUT_MS);
      });
      const rpcResult = await Promise.race([
        rpcPromise.finally(() => {
          if (rpcTimeoutId !== null) clearTimeout(rpcTimeoutId);
        }),
        rpcTimeout.finally(() => {
          if (rpcTimeoutId !== null) clearTimeout(rpcTimeoutId);
        }),
      ...
      if (rpcResult.reserves.length > 0) {
        if (rpcResult.errors.length > 0) {
          logger.warn(
            `⚠️ [V4] Layer 2 RPC fallback succeeded with ${rpcResult.errors.length} partial error(s): ${rpcResult.errors.join("; ")}`
          );
        }
        logger.info(
          `✅ [V4] Layer 2 RPC fallback succeeded: ${rpcResult.reserves.length} reserves`
        );
        return {
      ...
      if (rpcResult.errors.length > 0) {
        logger.warn(
          `⚠️ [V4] Layer 2 RPC fallback returned empty reserves with ${rpcResult.errors.length} error(s): ${rpcResult.errors.join("; ")}`
        );
      } else {
      ...
      logger.warn(
        `⚠️ [V4] Layer 2 RPC fallback failed: ${rpcError instanceof Error ? rpcError.message : String(rpcError)}`
      );
    }
  }
  ...
  logger.error(
    "❌ [V4] All fetch layers failed (SDK + RPC), returning empty dataset"
  );
  return {
    mapped: [],
    raw: { reserves: [] },
