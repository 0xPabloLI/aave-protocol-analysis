...
import {
  buildMarketsBaseDataset as _buildMarketsBaseDataset,
  buildV3BaseDataset as _buildV3BaseDataset,
  fetchV4ReservesWithTimeout as _fetchV4ReservesWithTimeout,
  fetchV3MarketsWithTimeout as _fetchV3MarketsWithTimeout,
  FETCH_TIMEOUT_MS,
} from "./concurrent-fetch.js";
import {
  fetchV4ReservesViaRpc,
  getDefaultV4SpokeEntries,
...
function buildChainTokenIndex(
  ...
  logger.info(
    `🗂️  Built chain-token index: ${Object.keys(index).length} chains`
  );
...
function getAllAaveV3NetworksFromAddressBook(): NetworkInfo[] {
  ...
  const aaveV3Networks = Object.keys(addressBook).filter(
    (key) =>
  ...
  const networkInfo: NetworkInfo[] = aaveV3Networks
    .map((networkName) => {
...
function extractChainsResult(
  ...
  if (
    ...
    "isErr" in result &&
    typeof (result as any).isErr === "function"
  ) {
    if ((result as any).isErr()) {
      const errorInfo = (result as any).error;
      throw new Error(
        errorInfo?.message || "Failed to fetch chains from Aave API"
      );
    }
    return ((result as any).value || []) as Array<{
      chainId: number;
      name?: string;
    }>;
  }
...
async function getAllAaveV3Networks(): Promise<NetworkInfo[]> {
  ...
  try {
    ...
    logger.info(
      `✅ Loaded ${networkInfo.length} chains from Aave API (MAINNET_ONLY)`
    );
    return networkInfo;
  } catch (error) {
    logger.warn(
      `⚠️ Failed to fetch chains from Aave API, falling back to address-book: ${error instanceof Error ? error.message : String(error)}`
    );
    return fallback;
  }
}
...
async function fetchBrevisAprs(
  ...
  try {
    ...
    baseDataset.forEach((reserve) => {
      ...
      if (!address) return;
      tokenPriceByChainAndAddress.set(
        chainTokenKey(reserve.chainId, address),
        price
      );
    });
    ...
    for (const [indexKey, campaigns] of Object.entries(brevisIndex)) {
      ...
      const enrichCampaignUsd = async (
        ...
        const reserveTokenPrice =
          tokenPriceByChainAndAddress.get(reservePriceKey);

        const breakdowns = await Promise.all(
          (campaign.breakdowns ?? []).map(async (breakdown) => {
            const normalizedAmount = toFiniteNumber(
              breakdown.budgetNormalizedAmount
            );
      ...
      campaigns.brevisSupplys = await Promise.all(
        campaigns.brevisSupplys.map((c) => enrichCampaignUsd(c))
      );
      campaigns.brevisBorrows = await Promise.all(
        campaigns.brevisBorrows.map((c) => enrichCampaignUsd(c))
      );
    }

    if (brevisResult.submitContracts.size > 0) {
      try {
        const chainCampaigns = Array.from(
          brevisResult.submitContracts.entries()
        ...
        logger.warn(
          `⚠️ Brevis distributedSoFar chain read failed: ${error.message}`
        );
      }
    }
    ...
    const totalSupply = Object.values(brevisIndex).reduce(
      (sum, item) => sum + item.brevisSupplys.length,
      0
    );
    const totalBorrow = Object.values(brevisIndex).reduce(
      (sum, item) => sum + item.brevisBorrows.length,
      0
    );
    ...
    logger.info(
      `✅ Indexed Brevis campaign data for ${Object.keys(brevisIndex).length} chain-token combinations`
    );
    logger.info(
      `   Supply campaigns: ${totalSupply}, Borrow campaigns: ${totalBorrow}`
    );
    logger.info(
...
export function buildMarketsBaseDataset(
  v3Markets: any[],
  v4Result: V4FetchResult
): ReturnType<typeof _buildMarketsBaseDataset> {
...
export async function fetchV3MarketsWithTimeout(options?: {
  ...
  return _fetchV3MarketsWithTimeout({
    _fetchV3Fn: options?._fetchV3Fn ?? fetchRawMarketData,
  });
}
...
export async function fetchV4ReservesWithTimeout(options?: {
  ...
  const fetchFn =
    options?._fetchV4Fn ??
    (() => fetchV4ReservesData({ throwOnFinalFailure: false }));
  // Production default: RPC direct-chain fallback via aave-rpc-infra (15s timeout enforced inside)
  const rpcFn =
    options?._fetchRpcFn ??
    (() =>
      fetchV4ReservesViaRpc({
        entries: getDefaultV4SpokeEntries(),
        timeoutMs: 15_000,
      }));
  return _fetchV4ReservesWithTimeout({
    _fetchV4Fn: fetchFn,
    _fetchRpcFn: rpcFn,
  });
}
...
type MerklProcessedData = {
  index: MerklDataIndex;
  campaignAccess?: import("@internal/aave-shared-contracts").MerklCampaignAccess[];
};
type BrevisProcessedData = {
  index: BrevisDataIndex;
  brevisDistributedSoFar: Map<string, number | undefined>;
};

function buildReserveTokenPriceMap(
  baseDataset: RuntimeReserveData[]
): Map<string, number> {
...
async function enrichDatasetWithIncentiveData(
  ...
  const symbolLookupCI = buildSymbolLookupCI(baseDataset);
  const equivLookup = buildEquivLookup();

  const llmApiKey = process.env.LLM_API_KEY;
  const llmBaseUrl = process.env.LLM_BASE_URL;
  const llmConfig: LlmClientConfig | undefined =
  ...
  const enrichedItems = await Promise.all(
    baseDataset.map(async (item) => {
      ...
      if (matchedOpportunities.length > 0) {
        ...
        for (const opp of matchedOpportunities) {
          const llmFn = llmConfig
            ? () => {
                ...
                const prompt = buildLlmPrompt({
                  ...
                  action: opp.name ?? opp.opportunityType ?? "unknown",
                  description: opp.description ?? "",
                  tokenSymbols: [item.tokenSymbol],
                });
                return callLlmWithFallback(prompt, llmConfig).then(
                  (outcome) => {
                    _llmCalls.push({
                      reserveId: item.reserveId,
                      elapsed: Date.now() - _llmStart,
                      outcome: outcome.tag,
                    });
                    if (outcome.tag === "unavailable") {
                      llmCircuitOpen = true;
                      logger.info(
                        `[enrich-llm] circuit opened after unavailable (${Date.now() - _llmStart}ms)`
                      );
                    }
                    return outcome;
                  }
            ...
            : undefined;
          const cachedConstraint = opp.opportunityId
            ? cachedConstraints?.get(opp.opportunityId)
            : undefined;
          const oppOffsetLevel: OffsetLevel = opp.opportunityType?.includes(
            "SPOKE_SUPPLY"
          )
            ? "reserve"
            : opp.opportunityType?.includes("HUB_SUPPLY")
              ? "hub-cross-spoke"
              : opp.opportunityType?.includes("V4_NET_APR")
                ? "hub-cross-spoke"
                : "reserve";
          const oppOffsetTokenAddresses = opp.offsetTokenAddresses;
          const netPositionConstraint = await detectNetPositionConstraint(
            opp,
            item.tokenAddress,
            item.reserveId,
            reserveIdSet,
          ...
          if (
            opp.opportunityId &&
            cachedConstraints &&
            cachedConstraint === undefined
          ) {
            cachedConstraints.set(
              opp.opportunityId,
        ...
        const dedupedSupply =
          deduplicateHubSpokeBreakdowns(supplyOpportunities);
        const dedupedBorrow =
          deduplicateHubSpokeBreakdowns(borrowOpportunities);
      ...
      if (brevisInfo) {
        if (
          brevisInfo.brevisSupplys.length > 0 ||
          brevisInfo.brevisBorrows.length > 0
        ) {
          item.brevisSupplys =
            brevisInfo.brevisSupplys.length > 0
              ? brevisInfo.brevisSupplys
              : undefined;
          item.brevisBorrows =
            brevisInfo.brevisBorrows.length > 0
              ? brevisInfo.brevisBorrows
              : undefined;
          if (isV4Reserve) {
            logger.warn(
              "V4 reserve matched Brevis incentive (expected V3-only)",
              {
                chainId: item.chainId,
                tokenSymbol: item.tokenSymbol,
                marketName: item.marketName,
      ...
      if (item.merklBorrows)
        item.merklBorrows = item.merklBorrows.map(pruneMerklGroup);
      if (item.merklHolds)
        item.merklHolds = item.merklHolds.map(pruneMerklGroup);
      if (item.brevisSupplys)
        item.brevisSupplys = item.brevisSupplys.map(pruneBrevisItem);
      if (item.brevisBorrows)
        item.brevisBorrows = item.brevisBorrows.map(pruneBrevisItem);
      return item;
    })
  );
  ...
  if (_llmCalls.length > 0) {
    ...
    const _llmCircuitOpen = _llmCalls.filter(
      (c) => c.outcome === "circuit-open"
    ).length;
    const _llmUnavailable = _llmCalls.filter(
      (c) => c.outcome === "unavailable"
    ).length;
    const _llmResult = _llmCalls.filter((c) => c.outcome === "result").length;
    logger.info(
      `[enrich-llm] calls=${_llmCalls.length} total=${_llmTotal}ms max=${_llmMax}ms unavailable=${_llmUnavailable} circuit-open=${_llmCircuitOpen} result=${_llmResult}`
    );
  }
...
async function fetchRawMarketData(): Promise<MarketData> {
  ...
  logger.info(
    `🌐 Found ${networkInfo.length} AaveV3 networks across ${chainIds.length} unique chains`
  );
  ...
  try {
    ...
    try {
      const result: any = await markets(createV3Client(), {
    ...
    const supportedChainIds = [
      ...new Set(marketList.map((m: any) => Number(m.chainId))),
    ];
    ...
    const marketData: MarketData = {
      ...
      networkInfo: networkInfo.filter((info) =>
        supportedChainIds.includes(info.chainId)
      ),
      markets: marketList,
      errors: errors,
    };
    ...
    logger.info(
      `📊 Found ${marketList.length} markets total from ${supportedChainIds.length} chains`
    );
    ...
    if (rateLimitStats.total429s > 0) {
      logger.warn(
        `📊 V3 rate limit: ${rateLimitStats.total429s} total 429s during this cycle`
      );
    }
  ...
  try {
    ...
    const [v3Settled, v4Settled] = await Promise.allSettled([
      ...
      fetchV4ReservesWithTimeout({
        _fetchV4Fn: () => fetchV4ReservesData({ throwOnFinalFailure: false }),
      }),
    ]);

    const v3Success = v3Settled.status === "fulfilled";
    const v4SettledValue =
      v4Settled.status === "fulfilled"
        ? v4Settled.value
        : {
            mapped: [],
            raw: { reserves: [] },
    ...
    if (!v3Success) {
      const reason =
        v3Settled.status === "rejected" ? v3Settled.reason : "unknown";
      logger.error(
        `❌ V3 fetch failed: ${reason instanceof Error ? reason.message : String(reason)}`
      );
    }
    if (!v4Success) {
      logger.warn(
        `⚠️ V4 fetch produced no data (source=${v4SettledValue.source})`
      );
    }
    ...
    const { baseDataset, v3Count, v4Count } = buildMarketsBaseDataset(
      marketData.markets,
      v4Result
    );
    ...
    const timeoutPromise = new Promise<{
      ...
      timeoutId = setTimeout(async () => {
        ...
        logger.warn(
          `⏱️ Incentive data fetching timeout after ${INCENTIVE_DATA_TIMEOUT_MS / 1000}s, extracting completed results...`
        );

        // 使用一个很短的超时（100ms）来检查每个 Promise 是否已完成
        const checkCompleted = async <T>(
          promise: Promise<T>,
          defaultValue: T
          ...
          try {
            const result = await Promise.race([
              ...
              new Promise<{ completed: false; value: T }>((resolve) =>
                setTimeout(
                  () => resolve({ completed: false, value: defaultValue }),
                  100
                )
              ),
            ]);
        ...
        const merklData: MerklDataIndex = merklCheck.completed
          ...
          : {};
        const brevisData: BrevisDataIndex = brevisCheck.completed
        ...
        logger.warn(
          `   • Brevis: ${brevisCheck.completed ? "completed" : "timeout/empty"}`
        );
        logger.warn(
    ...
    logger.info(
      `🎯 Final dataset contains ${enrichedData.length} token combinations`
    );

    // 保存格式化 JSON 数据（完整 debug 全量）
    const debugFormattedJsonFullPath = join(
      DEBUG_DATA_DIR,
      "v3v4-enriched-full.json"
    );
    const debugPayload = {
      _metadata: {
    ...
    logger.info(
      `🧪 V3+V4 enriched JSON saved to ${debugFormattedJsonFullPath}`
    );
...
function launchIncentiveFetches(
  ...
  const merklPromise = processMerklData({
    ...
    logger.error(
      `❌ Merkl data fetching failed: ${error instanceof Error ? error.message : String(error)}`
    );
    return { index: {} as MerklDataIndex } as MerklProcessedData;
  });
  const brevisPromise = fetchBrevisAprs(baseDataset).catch((error) => {
    logger.error(
      `❌ Brevis data fetching failed: ${error instanceof Error ? error.message : String(error)}`
    );
    return {
      index: {} as BrevisDataIndex,
      brevisDistributedSoFar: new Map<string, number | undefined>(),
...
export async function fetchMarketsData(options?: {
  ...
  const { baseDataset, v3Count, v4Count, v4Raw, spokeHubTopology } =
    buildMarketsBaseDataset(marketData.markets, v4Result);
  const reserveTokenPriceByChainAndAddress =
    buildReserveTokenPriceMap(baseDataset);
  logger.info(
  ...
  logger.info(
    `Enriching done: ${runtimeData.length} reserves [enrich=${Date.now() - _enrichStart}ms, total=${_elapsed()}]`
  );

  logger.info(
    `🎯 Final dataset contains ${runtimeData.length} reserves [${_elapsed()}]`
  );
  ...
  const payload: MarketsPayload = {
    ...
    data: runtimeData,
    ...(merklResult.campaignAccess?.length
      ? { campaignAccess: merklResult.campaignAccess }
      : {}),
    ...(spokeHubTopology.length ? { spokeHubTopology } : {}),
    ...(brevisResult.brevisDistributedSoFar.size > 0
      ? { brevisDistributedSoFar: brevisResult.brevisDistributedSoFar }
      : {}),
  };

  // Write debug files (non-blocking, never fail the cron)
  writeDebugSnapshot(payload, runtimeData, v3Count, v4Count, v4Raw).catch(
    (err) => {
      logger.warn(
        `⚠️ Debug file write failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`
      );
    }
  );
...
async function writeDebugSnapshot(
  ...
  await writeJsonAtomic(join(DEBUG_DATA_DIR, "v3v4-enriched-full.json"), {
    _metadata: {
      ...payload._metadata,
      version: "2.0-debug-full",
      profile: "debug-full",
  ...
  if (v4Raw.reserves.length > 0) {
    ...
    await writeFile(
      join(DEBUG_DATA_DIR, "v4-raw-sdk-response.json"),
      rawJson,
      "utf-8"
    );
  }
