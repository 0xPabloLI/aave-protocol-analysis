...
test('FIELD_UNITS covers every field in RuntimeReserveData', () => {
  ...
  assert.deepEqual(missing, [], `Fields in RuntimeReserveData but NOT in FIELD_UNITS: ${missing.join(', ')}. Add them to FIELD_UNITS in units.ts.`);
  assert.deepEqual(extra, [], `Fields in FIELD_UNITS but NOT in RuntimeReserveData: ${extra.join(', ')}. Remove them or add to EXPECTED_RUNTIME_FIELDS.`);
});
...
test('RATIO_FIELDS and PERCENT_FIELDS partition FIELD_UNITS correctly', () => {
  ...
  for (const field of allFields) {
    ...
    if (unit === 'ratio') {
      assert.ok(ratioSet.has(field), `Field "${field}" is 'ratio' but not in RATIO_FIELDS`);
      assert.ok(!percentSet.has(field), `Field "${field}" is 'ratio' but also in PERCENT_FIELDS`);
    } else if (unit === 'percent') {
      assert.ok(percentSet.has(field), `Field "${field}" is 'percent' but not in PERCENT_FIELDS`);
      assert.ok(!ratioSet.has(field), `Field "${field}" is 'percent' but also in RATIO_FIELDS`);
    }
  }
});
...
test('Critical: supplyApy and borrowApy are ratio fields', () => {
  assert.equal(FIELD_UNITS.supplyApy, 'ratio', 'supplyApy MUST be ratio — serializer applies ×100');
  assert.equal(FIELD_UNITS.borrowApy, 'ratio', 'borrowApy MUST be ratio — serializer applies ×100');
});

test('Critical: baseBorrowRate is a percent field (not ratio)', () => {
  assert.equal(FIELD_UNITS.baseBorrowRate, 'percent', 'baseBorrowRate MUST be percent — serializer passes through');
});
