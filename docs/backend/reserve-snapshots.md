...
market_snapshots (
  ...
  incentive_details       JSONB,               -- 完整 breakdown（merkl/brevis）
...
后端 `persistenceService.ts` 的 `buildSnapshotRow()` 在每次快照时已经将激励数据写入 `incentive_details` JSONB 列，包含 per-campaign 级别的 merkl/brevis 细分数据。
...
- `incentive_details` JSONB 内部结构对齐 `MerklOpportunityGroup` / `BrevisIncentive`
