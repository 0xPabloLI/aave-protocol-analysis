...
**描述**: 聚合返回低频侧数据，用于前端一次性获取 categories、FDV 快照、forecast 状态（Merkl + Brevis 合并）和 campaign 白名单。内部组合了：

- `categories` 子块（6h TTL）
- `fdv` 子块（5m TTL）
- `forecast` 子块（Merkl + Brevis，10m TTL）
...
- `500`: categories、fdv、forecast、campaignAccess 均失败（无可用侧数据）。
...
### 4. Brevis forecast contract（已合并入 forecast 子块）

Brevis forecast 数据已合并到 `GET /api/meta/side-data` 的 `forecast.items[]` 中，与 Merkl forecast 共享同一个数组。
