...
| #   | 文件                      | 变量                                 | 类型      | Domain | TTL                     | Max                 | Shrink                             | 结论                                                                                                                                                       | 不需要理由                                                                                                                                                                                                 |
| --- | ------------------------- | ------------------------------------ | --------- | ------ | ----------------------- | ------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| —   | merit-api                 | _(已整体删除)_                       | —         | —      | —                       | —                   | ⚪                                 | 原 #27–#30（roundEstimateCache / campaignMetadataMemoryCache / meritCurrentBlockNumberCache / discoveredRedirectAliases）随 Merit 下线一并移除（AAV-1289） |
...
for (const key of map.keys()) {
  /* 期望 key 是 chainId|tokenAddr|configId 格式，但无编译时保证 */
}
...
> 以下为按日期归档的历史会话记录（原文保留）。其中提到的 `merit-api.ts` / `cloudflare-browser.ts` /
> Cloudflare Worker 已于 2026-09 随 Merit 下线删除（AAV-1289），引用的符号不再存在。
