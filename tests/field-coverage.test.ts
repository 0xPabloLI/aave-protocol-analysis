...
const mockFullReserve = {
  ...
  hubName: "Core",
  hubAddress: "0x1234",
  hubBorrowed: "500000000000",
  hubSupplied: "900000000000",
  spokeId: "1",
  spokeName: "Main",
  spokeAddress: "0x5678",
  ltv: 82.5,
...
test("all critical fields defined in EXPECTED_RUNTIME_FIELDS", () => {
  ...
  for (const field of criticalFields) {
    assert(
      EXPECTED_RUNTIME_FIELDS.includes(field as any),
      `Missing field: ${field}`
    );
  }
});
...
test("detect missing fields in runtime data", () => {
  ...
  assert(
    missing.includes("tokenAddress"),
    "Should detect missing tokenAddress"
  );
...
test("Phase 2: new field names registered in EXPECTED_RUNTIME_FIELDS", () => {
  ...
  assert.strictEqual(
    missing.length,
    0,
    `New field names not in registry: ${missing.join(", ")}`
  );
});
...
test("Phase 2: old field names removed from EXPECTED_RUNTIME_FIELDS", () => {
  ...
  assert.strictEqual(
    stillPresent.length,
    0,
    `Old field names still in registry: ${stillPresent.join(", ")}`
  );
});
...
test("Phase 2: new field names present in mock reserve", () => {
  ...
  assert.strictEqual(
    missing.length,
    0,
    `New fields missing from mock: ${missing.join(", ")}`
  );
});
...
test("Phase 2: old field names removed from mock reserve", () => {
  ...
  assert.strictEqual(
    stillPresent.length,
    0,
    `Old field names still in mock: ${stillPresent.join(", ")}`
  );
});
